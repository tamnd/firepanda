...
def test_a_separator_is_a_name_re2_has_and_the_table_once_had_not() raises:
    """The table is generated by asking RE2 about each name on a written list,
    and the list had been written from memory without the separators on it, so
    `\\p{Zs}` was refused with the sentence RE2 has no such character class
    about a class RE2 has and pandas answers with a column. Document 115."""
    var space = compile_program(parse_pattern("\\p{Zs}"), ENGINE_RE2)
    assert_true(space.ok)
    assert_true(matches_text(space, " "))
    assert_false(matches_text(space, "a"))

    # The union is the three of them and nothing else, so a tab is outside it,
    # which is the one a reader guesses wrong: a tab is a control character.
    var whole = compile_program(parse_pattern("\\p{Z}"), ENGINE_RE2)
    assert_true(whole.ok)
    assert_true(matches_text(whole, " "))
    assert_true(matches_text(whole, String(chr(0x2028))))
    assert_true(matches_text(whole, String(chr(0x2029))))
    assert_false(matches_text(whole, "\t"))

    var line = compile_program(parse_pattern("\\p{Zl}"), ENGINE_RE2)
    assert_true(line.ok)
    assert_true(matches_text(line, String(chr(0x2028))))
    assert_false(matches_text(line, String(chr(0x2029))))
