...
def test_a_logical_type_stays_one_cache_line() raises:
    """The size is asserted because a field in the wrong place was miscompiled.

    A type is copied constantly, into every field of every schema and every
    value, so sixty four bytes rather than eighty is worth having on its own.
    The reason it is a test is the other thing that happened. With the ordered
    flag declared after the zone, where a new field naturally goes, the type
    came to eighty bytes and a value built as a temporary and passed to a
    constructor with a defaulted trailing argument arrived with the flag reading
    as a byte of stack. Two float64 types then compared unequal and five
    pipeline tests failed, none of them naming anything to do with this file. If
