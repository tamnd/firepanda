...
def test_a_pattern_python_answers_and_re2_refuses_is_still_a_gap() raises:
    ...
    backreference is a pattern pandas answers and this engine does not, and
    saying so is the whole of what the flag is for.

    The lookahead was this row's example until document 93 answered it and the
    lookbehind was until document 94 did, which is two of the five constructs
    moving out of this list."""
    var program = compile_program(parse_pattern("(a)\\1"), ENGINE_PYTHON)
    assert_false(program.ok)
    assert_true(program.gap)
