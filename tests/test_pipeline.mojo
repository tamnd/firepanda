...
def narrow_frame() raises -> DataFrame:
    """A hundred rows of an int8 and an int32 in five chunks, with gaps.

    The int8 runs over its whole range, so ten added to it wraps in some chunks
    and not in others, and a reduction that answers a sum of `x + c` from the
    sum of `x` has to notice which. Every seventh row is missing in both.
    """
    var small = ChunkedArray(LogicalType.INT8)
    var wide = ChunkedArray(LogicalType.INT32)
    for c in range(5):
        var a = Array[DType.int8](20)
        var b = Array[DType.int32](20)
        for i in range(20):
            var row = c * 20 + i
            a[i] = Int8(((row * 37) % 256) - 128)
            b[i] = Int32((row * 7919) % 200001 - 100000)
            if row % 7 == 3:
                a.set_null(i)
                b.set_null(i)
        small.append(AnyArray(a^))
        wide.append(AnyArray(b^))
    var columns = List[ChunkedArray]()
    columns.append(small^)
    columns.append(wide^)
    var fields = List[Field]()
    fields.append(Field("small", LogicalType.INT8))
    fields.append(Field("wide", LogicalType.INT32))
    return DataFrame(Schema(fields^), columns^)


def test_a_sum_carrying_an_operation_is_the_sum_of_what_it_made() raises:
    """Sums and counts of a column under a constant, which the node answers
    from the column's own sum and count where it can, held against the same
    numbers worked out a row at a time here. `small + 100` wraps in int8 on
    most chunks, and there the node has to build the column after all, so the
    wrapped answer is the one it has to give."""
    var aggs = List[GroupAgg]()
    aggs.append(GroupAgg(1, AggKind.SUM, "plus", BinaryOp.ADD, Value(1000)))
    aggs.append(GroupAgg(1, AggKind.SUM, "minus", BinaryOp.SUB, Value(7)))
    aggs.append(GroupAgg(1, AggKind.SUM, "from", BinaryOp.SUB, Value(7), True))
    aggs.append(GroupAgg(1, AggKind.SUM, "times", BinaryOp.MUL, Value(-3)))
    aggs.append(GroupAgg(1, AggKind.COUNT, "seen", BinaryOp.ADD, Value(1)))
    aggs.append(
        GroupAgg(
            1,
            AggKind.SUM,
    ...
    aggs.append(GroupAgg(0, AggKind.SUM, "wraps", BinaryOp.ADD, Value(100)))
    aggs.append(GroupAgg(0, AggKind.SUM, "less", BinaryOp.SUB, Value(1)))
    aggs.append(
        GroupAgg(0, AggKind.SUM, "wide", BinaryOp.ADD, Value(Int64(100)))
    )
    var pipeline = Pipeline(narrow_frame())
    pipeline.add(Node(Reduce(aggs^)))
    var out = pipeline^.run()
    ...
    for row in range(100):
        if row % 7 == 3:
            continue
        var a = Int8(((row * 37) % 256) - 128)
        var b = Int32((row * 7919) % 200001 - 100000)
        plus += Int64(b + 1000)
        minus += Int64(b - 7)
        from_ += Int64(7 - b)
        times += Int64(b * -3)
        ...
        less += Int64(a - 1)
        wide += Int64(a) + 100
    assert_equal(one_int(out, "plus"), plus, "x + 1000")
    assert_equal(one_int(out, "minus"), minus, "x - 7")
    assert_equal(one_int(out, "from"), from_, "7 - x")
    assert_equal(one_int(out, "times"), times, "x * -3")
    assert_equal(one_int(out, "seen"), seen, "the values that were there")
    assert_equal(one_int(out, "marked"), plus - 990 * seen, "x + 10")
    assert_equal(one_int(out, "wraps"), wraps, "x + 100, wrapped in int8")
    assert_equal(one_int(out, "less"), less, "x - 1, wrapped in int8")
    assert_equal(one_int(out, "wide"), wide, "x + 100 in int64")
