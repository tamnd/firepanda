"""Tests for the three window reductions that measure a spread.
...
The exceptions are the two families where pandas is measurably wrong, which are a
window holding an infinity and a window whose sum of squared deviations has
overflowed. Those are checked against what is true and the tests say so and say
why. `firepanda/kernel/spread.mojo` carries the argument.
"""
...
from firepanda.kernel.spread import SPREAD_SEM, SPREAD_STD, SPREAD_VAR, Spread
...
def test_a_rolling_variance_over_consecutive_rows_is_the_same_every_row() raises:
    ...
    five, which it should, because every window holds three rows a step apart and
    the variance does not care where they sit.
    """
...
def test_the_deviation_is_the_root_of_the_variance() raises:
    ...
    1.5811388300841898]`, which is the root of `[nan, 0.5, 1, 1.6666666666666667,
    2.5]` row for row.
    """
