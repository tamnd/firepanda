...
This dictionary used to be written from memory and the separators were left out
of it, so `\\p{Zs}` was a name RE2 takes and this table had never heard of, and a
caller who wrote one got told RE2 has no such class by a library that had simply
not been asked. `sweep` is what stops that happening again. Document 115."""
...
The separator names are here because they were once not. `\\\\p{{Zs}}` and
`\\\\p{{Zl}}` and `\\\\p{{Zp}}` and their union are all names RE2 takes, and the
list the generator was given had been written from memory without them, so this
library answered a caller who wrote one by saying RE2 has no such class about a
class RE2 has. The generator now asks RE2 about every name of one letter and
every name of a letter and a lower case letter rather than being told which ones
there are. Document 115.
...
name RE2 takes, that each one letter category is exactly the union of its two
letter parts, and that no short name RE2 takes is one it was never told about,
before it writes anything, so a release that moves any of the three fails here
...
def sweep() -> None:
    """Every short name RE2 takes, found by asking rather than by remembering.

    A general category name is one letter or a letter and a lower case letter,
    and there are 702 of those, and asking RE2 about each one costs a compile of
    a pattern that never runs. That is a second or two, against a list somebody
    wrote out by hand and got wrong once already. Two letter script names are
    caught in the same net, which is why the comparison is against the scripts
    ...
    for first in letters:
        for second in letters:
            candidates.append(first + second.lower())
    takes = {name for name in candidates if reads("\\p{%s}" % name)}
    missing = sorted(takes - set(PAIRS) - set(PARTS) - set(SCRIPTS))
    if missing:
        raise SystemExit(
            "RE2 takes names this generator was never told about: "
            + " ".join(missing)
        )
...
def kept(out: Path) -> str:
    """Whatever the file being rewritten had below the tables.

    The three lookups at the bottom of `unicodedata.mojo` are written by hand
    and this generator does not know how to write them, so a run that simply
    wrote the tables deleted them and the next build failed. Reading them back
    off the file is the smallest fix that cannot be forgotten, and a file that
    has nothing below the tables, which is what a fresh `--out` is, keeps that
    property.

    Args:
        out: The file about to be written.

    Returns:
        The text from the first definition onwards, or nothing.
    """
    if not out.exists():
        return ""
    was = out.read_text()
    at = was.find("\n\n\ndef ")
    return was[at + 1 :] if at >= 0 else ""
...
def main() -> int:
    ...
    args.out.write_text(body + emit(table) + kept(args.out))
    print(sentence)
    return 0
