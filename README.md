...
| `type-literal` | a typed literal such as DATE '2020-01-01' | The type in front of the string decides how the string is read, which is a cast, and a cast is written CAST(x AS t) here. The types a cast reaches are the boolean, the eight fixed width integers, FLOAT, DOUBLE and VARCHAR, so a date literal waits on the date work either way. | [#304](https://github.com/tamnd/firepanda/issues/304) |
