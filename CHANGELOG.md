...
- Bool, string, binary and null are refused by name with the reason, rather than exported as something a consumer would misread. A firepanda bool column is a byte per value where Arrow wants a bit, and a string column is the view layout, which needs the variadic data buffers and a buffer count that is not a constant. Both are conversions, so both are follow-ups.
