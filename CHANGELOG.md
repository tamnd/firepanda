...
The planner specification is ten documents and it argues one thing: the engine today makes its interesting decisions in the wrong place, either from a parameter name, or from a constant with a row count in it, or not at all. Which side of a join is built is decided by which parameter is called right. Whether a filter runs before or after a join is decided by the order the user wrote them in. The first stage of the milestone it opens is the set of those decisions that need no plan layer to fix, and the compound key join above is the first of them.
