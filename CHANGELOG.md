...
Measured on an i9-13900K, scale factor one lineitem, six million rows, all three routes agreeing on revenue 123141078.2283. Written by hand against the eager API, 19.2 ms. The same query as a plan, lowered and run over the frame in one chunk, 18.6 ms. The same plan over the same read asked for in morsels of a hundred and thirty one thousand rows, 10.9 ms. Those are the best of three process runs of best of seven; the machine is noisy and the ratio moved between 1.4x and 1.8x across runs, but no run had the morselled route slower.
