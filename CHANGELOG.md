...
Two things are honestly not done. The parse is six times over its budget: on an M4, optimized, TPC-H q1 takes 380 microseconds against a target of 60 and a DuckDB measurement of 156, and `SELECT 1` takes 20 microseconds against a target of 3. The tokenizer is not the problem, at 1.7 microseconds for q1 and 64 nanoseconds for `SELECT 1`, and neither is allocation, which is five per cent of the parse. It is 27,814 node visits and 10,550 rule entries for one hundred and twenty tokens, so the fix is to visit fewer nodes rather than to shave the visit, and that is its own piece of work.
