...
### Fixed: the README's SQL support table matches the refusals again

Reading a window specification changed what the `call-modifier` refusal says and the generated table in the README was not regenerated with it, so `test_the_readme_table_is_the_refusal_table` failed on main and on every branch that merged it. The table is generated by `pixi run sql-support` and the fix is to run it, which is the whole change.
