...
That curve is a property of one machine's cache and core count, which is why there is no default height in this file and the caller names one. What is still unmeasured is how it moves on a machine with fewer cores. Every attempt at an eight core cross check so far has been on a machine that was running somebody else's benchmark, so there is no number worth quoting for it yet.
