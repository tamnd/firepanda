...
The first measurement timed only the whole five condition predicate of TPC-H q6, saw the two routes overlap, and called it a wash. That is the one point on the curve where they meet. Timing each condition count separately, over six million rows on a thirty two thread desktop, best of fifteen runs, milliseconds: with one condition 6.5 before and 38.7 with the selection, with two 7.1 and 16.0, with three 7.4 and 13.0, with four 7.7 and 10.1, with five 7.9 and 7.7. One condition keeps seven rows in ten and five keep about one in fifty, so the selection is six times slower when a predicate keeps most of its input and only breaks even where the answer has nearly vanished. Filtering is not usually the second of those.
...
`Series.ewm` and `DataFrame.ewm` take the decay as exactly one of `com`, `span`, `halflife` and `alpha`, and the four are four spellings of one number. The collapse happens on the Python side, so pandas' own sentences for a decay that is missing, doubled or out of range are the sentences a caller gets, and one number crosses the boundary. It is checked again at the kernel's door, which is the arrangement `closed` already has and is there because the Mojo API is also a caller and does not come through Python.

`adjust` and `ignore_na` are one loop and not two. Under `adjust=True` an arriving row weighs one against everything before it, under `adjust=False` it weighs the smoothing factor and the weight behind it is reset after every row, and that one number is the whole of the difference between the adjusted form and the plain recursion. `ignore_na` decides whether a missing row still takes a decay step, and crossed with `adjust` that is four genuinely different answers over a column with gaps in it rather than four roundings of one. All four are pinned in the Mojo tests and checked against a running pandas at the boundary.

`mean`, `sum`, `var` and `std` are here, with `bias` on the two spreads and `min_periods` on all four. A corrected weighted variance needs two weight totals and not one, the sum of the weights and the sum of their squares, each with its own decay, and the denominator of the correction being zero is why the first row of a corrected variance is missing where the first row of a biased one is nought. `sum` with `adjust=False` raises `NotImplementedError` with pandas' sentence, because pandas does not choose between the two things such a total could mean and inventing an answer would be the one place in this family where firepanda answers something pandas does not.

One line is copied from pandas rather than derived. Its fold skips the update entirely when the arriving row already equals the carried mean, which is there to keep a constant column constant, and without it a flat column has visibly drifted after a few hundred rows. The behaviour it produces is the correct one and the version without it is not, so it is here, comparison and all.

Two of the kernel tests were written against pandas' answers and failed by one unit in the last place, with an identical recurrence on both sides. Three algebraic orderings of the fold were measured against pandas and none reproduced it, and the fourth attempt, using a fused multiply-add, reproduced it exactly. The pandas wheel these were measured against was compiled with the multiply and the add of the fold contracted into one instruction, which rounds once where two operations round twice. That is not reproduced. A kernel whose specification is another kernel's choice of instruction is a kernel nobody can maintain, so the tests carry a relative tolerance of `1e-15` and both docstrings say why.

Three refusals are stricter than pandas on purpose. `adjust`, `ignore_na` and `bias` are read for truth by pandas, so `adjust="no"` is accepted there and means yes, and all three are required to be booleans here, which is the position `center` and `pct` already took on the rolling side. `times` and `method="table"` are declared so the signature accepts what pandas accepts and refused by name, and `numeric_only` on a frame is refused rather than answered, because which columns come back is a decision this library does not make.
...
It works out one set per node, of the output positions anything above it reads, in a single walk from the root downwards. Node indices are handed out in creation order so an input always sits below the node reading it, which means counting down reaches every node after everything that demands anything of it, with no recursion and no visited set. Every node needs what the node above asked of it plus whatever its own expressions read, and those two are not the same question: a filter's predicate reads a column that nothing above the filter wants, and that column still has to arrive. Getting that wrong makes queries wrong rather than slow, so there is a test for it on each of the nodes that has expressions of its own.

Three of the nine kinds carry a list of columns and only those three are rewritten. A scan gets its column list narrowed, which is where the reading stops happening, and a q6 shaped plan over sixteen columns of lineitem comes out reading four. A project and an aggregate get their output lists narrowed, because a node that still computes a column still has to read what that column is made of, and an unused output of a project is an expression evaluated once per row for nobody. A filter, a sort, a limit, a distinct, a join and a union hand their input's columns through unchanged and have nothing on them to narrow.

Two things it will not do, and both are load bearing. An aggregate keeps every group key whatever anything above thinks of it, because dropping a key is not a projection, it is a different query with different rows in it. And a project or an aggregate with two outputs of the same name is left exactly as it was, because binding resolves a name to the first column that has it and dropping the first of two would silently point an expression above at the other one. That plan is ambiguous before the pass sees it, and declining to make it worse is better than the pass having an opinion about which of the two was meant.

The pass rebinds rather than remapping. A position only means something against a schema and this pass changes the schemas, so instead of rewriting every bound position by hand it hands the plan back to `bind`, which resolves by name and gets the new positions right for nothing. `bind_all` is the small piece that made that practical: it is the whole of what binding already worked out rather than only the root's schema, which is what a pass needs when it has to know the width of what a join's left input produces. Binding a plan twice at plan time costs nothing next to reading one column nobody wanted.

Nineteen tests, most of them a plan in and a scan's column list out, because that list is what the pass exists to shorten and is what a reader can check by eye. `Expressions.positions` is new beside `tables` and is the same analysis one level finer: `tables` says which relations an expression needs and this says which of their columns, so a pass that has both can decide what a subtree has to produce.
...
### Added: a read that can come back in morsels, so the pipeline has something to divide

The assembler allocated one sink per column and filled it from every batch at once, which is the right shape for a read and the wrong shape for everything after it. The pipeline runs its elementwise operators a chunk per worker, so a frame that arrived as a single allocation is a query on one core however many the machine has. That was measured rather than guessed: TPC-H q6 lowered into the pipeline took the same time as the eager route until the frame was cut up, and then it took two fifths of it.

Cutting one up afterwards writes every byte a second time. Putting the boundaries in while the first copy is happening costs nothing, because the assembler already divides its work into pieces of sixty five thousand rows and fills them in parallel. A piece is now trimmed so that it never crosses a morsel boundary, and it writes into its own morsel's sink at an offset instead of into a long sink at a larger one. The single chunk case is the same code with one morsel, so nothing about the default read changed.

Measured on an idle i9-13900K with thirty two threads, scale factor one lineitem, six million rows, every route agreeing on revenue 123141078.2283. Written by hand against the eager API, 19.6 ms. The same query as a plan, lowered and run over the frame in one chunk, 19.2 ms, and the two being the same number is the point rather than a coincidence. The same plan over the same read asked for in morsels of sixteen thousand rows, 7.9 ms, which is 2.5x. Best of seven, and two runs of the whole probe agreed to within half a millisecond everywhere.

The height matters and what it has is a plateau rather than a peak. Sweeping from four thousand rows to a quarter of a million: 11.7 ms at four thousand, 9.2 at eight thousand, 8.2 at sixteen thousand, 8.2 at thirty two thousand, 8.7 at sixty five thousand, 12.4 at a hundred and thirty one thousand and 13.0 at a quarter of a million. Anything between eight and sixty five thousand is within ten percent of the best and it falls away in both directions outside that, because too small means the per chunk work starts to show and too large means a morsel of four float64 columns has stopped fitting in a core's L2.

That curve is a property of one machine's cache and core count, which is why there is no default height in this file and the caller names one. The eight core cross check says so plainly, because on eight cores the curve points the other way. Same query and same six million rows on an eight core EPYC VM, three runs, the third of them on a machine with a load average of 0.05: morsels of four thousand and of eight thousand rows are slower than one chunk on all three runs, and the quarter of a million row morsel is the fastest on all three, at 1.30x, 1.45x and 1.66x. Sixteen and thirty two thousand, which is the best height on the thirty two thread machine, sit between 0.82x and 1.23x here.

Two things follow from that. The best height is not a constant, and a default baked into this file would be wrong on one of the two machines whatever it was set to. And the ceiling on what morselling buys tracks the core count rather than anything about the plan, which is what you would expect from a change whose whole effect is to let more than one core work on a query: 2.5x on thirty two threads and about 1.5x on eight. The absolute numbers on that VM bounce by about twenty percent run to run even when it is quiet, so the shape is worth quoting and the individual milliseconds are not.

The read itself did not get slower, it got faster. Alternating the two reads three times and keeping the best of each, one chunk was 60.3 ms and morsels 54.0 ms. Reading them in alternation is the point of that measurement: whichever read went first paid for DuckDB waking up and for the file not being in the page cache, and it would have looked slower for reasons that have nothing to do with chunking.

The height is on `Session.run` and not on `read_parquet`, deliberately. `DataFrame` borrows a column by position only when there is a single chunk to borrow, so an eager caller handed a morselled frame would find `frame[0]` raising. A morselled frame is for the pipeline, and the public reader keeps handing back the frame the eager API needs until the plan layer is what runs a query. A read carrying a list or struct column stays in one chunk either way, because a nested column is built whole and a frame whose columns disagree about their boundaries is not a frame.

Ten tests, over five thousand rows, which is enough that DuckDB's own chunks of two thousand and forty eight cross the morsel boundaries rather than lining up with them. The ones that matter are the null that lands after a boundary and the string too long to inline, because validity is pasted after the fill rather than during it and a string column plans its payload before it allocates, and both of those totals are now per morsel.
...
Grouping and whole frame reduction lower too, to `Group` and `Reduce` respectively. Choosing between them on whether there are keys is the one physical choice this file makes, and it is the choice the plan node's own docstring already said was physical: knowing there is one group rather than hashing every row to find out is not a difference in what the query means. That is enough for a TPC-H q6 shaped query, a scan and a conjunction of three range predicates and a product and a sum, to lower and run end to end.

What it will not lower, it refuses by name. Joins, sorts, distincts and unions have no operator here yet, and neither does a unary expression, a conditional, a window, a limit that skips rows first, a projection that renames a column, or a cast of an input column, which would convert that column where it lies and change what its position means for everything bound against it. Each of those raises an error saying what it was, and a caller that gets one keeps whatever route it had. It does not fall back to `Materialize`, because `Materialize` holds a function pointer that captures nothing and there would be no way to hand it the expression that could not be lowered.
...
TPC-H q6 at scale factor one, on an idle i9-13900K, six million rows of lineitem, both routes agreeing on revenue 123141078.2283. Written by hand against the eager API, which is five masks over every row anded together and one filter, 19.6 ms. The same query as a plan, bound, simplified, lowered and run, 19.2 ms. Those being the same number is the finding rather than a disappointment, and it is worth saying plainly: lowering a plan buys nothing on its own here. The fourteen operators the plan produces are strictly less work than the five full width masks the hand written route computes, and none of that shows, because the driver runs its elementwise prefix one chunk per worker and the frame it is given arrives in one chunk. One chunk is one core, on a machine with thirty two of them.

What that says is where the next change has to go, and it is the read rather than the plan. The same fourteen operators over the same rows in chunks of sixteen thousand take 7.9 ms, which is 2.5x, and getting the frame into that shape without copying every byte a second time is a separate piece of work. What none of this yet reaches is the fused single pass at 1.25 ms or DuckDB 1.5.5 at 2.64 ms, and that remaining gap is the per operator materialisation that a selection vector and operator fusion are there to remove.
