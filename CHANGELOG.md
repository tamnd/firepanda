...
Measured on an i9-13900K, scale factor one lineitem, six million rows, all three routes agreeing on revenue 123141078.2283. Written by hand against the eager API, 19.2 ms. The same query as a plan, lowered and run over the frame in one chunk, 18.6 ms. The same plan over the same read asked for in morsels of a hundred and thirty one thousand rows, 10.9 ms. Those are the best of three process runs of best of seven, and the ratio moved between 1.4x and 1.8x across them.

Two things about that measurement are not settled and are worth saying rather than leaving to be discovered. Which height is fastest is one of them: a sweep from four thousand to a quarter of a million rows never once put a hundred and thirty one thousand at the top, and pointed somewhere between eight and sixty five thousand, which is about where a morsel stops fitting in L2. The other is how much of this is a property of a machine with thirty two threads. A first look on an eight core virtual machine had the morselled route no faster than the eager one. Both machines were running somebody else's benchmark at the time, so neither of those is a number to act on yet, and that is why nothing here has a default height and the caller names one.

The read itself did not get slower, it got faster. Alternating the two reads three times and keeping the best of each, one chunk was 70.7 ms and morsels 63.7 ms. Reading them in alternation is the point of that measurement: whichever read went first paid for DuckDB waking up and for the file not being in the page cache, and it would have looked slower for reasons that have nothing to do with chunking.
