...
struct LogicalType(Equatable, ImplicitlyCopyable, Movable, Writable):
    ...
    It is declared here, among the one byte fields, rather than at the end where
    a new field would naturally go, and the position is load bearing for two
    reasons. The zone below is two sixteen byte SIMD vectors and is aligned to
    sixteen, so a byte placed after it starts a second block of padding and
    takes the whole type from sixty four bytes to eighty for the sake of one
    bit. Here it costs nothing, since the three fields above leave thirteen
    bytes of padding before the zone begins.

    The second reason is that the eighty byte version was miscompiled. With the
    flag declared after the zone, a `Value` built as a temporary and passed to a
    constructor that has a defaulted trailing argument arrived with the flag set
    to a byte of stack rather than to what was written into it, which made two
    float64 types compare unequal and broke five pipeline tests in a way that
    named nothing about categoricals. Passing the argument explicitly, or moving
    a named value in, avoided it. `test_a_logical_type_stays_one_cache_line`
    fails if the layout ever grows back."""
    ...
    def timestamp(unit: TimeUnit, zone: TimeZone = TimeZone()) -> Self:
        ...
        Returns:
            The type.
        """
        return Self(TypeKind.TIMESTAMP, DType.int64, unit, False, zone)
    ...
    def duration(unit: TimeUnit) -> Self:
        ...
        Returns:
            The type.
        """
        return Self(TypeKind.DURATION, DType.int64, unit, False, TimeZone())
