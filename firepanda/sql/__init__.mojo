...
data rather than source: 40 `.gram` files, 5 keyword lists, the MIT license they
come under, and a `VENDOR` file recording the commit they came from and the
SHA-256 of each one. `tools/vendor_grammar.sh` is the only thing allowed to write
into it.
...
from .table import Grammar, GrammarNode, memoized_rules
