...
For Parquet, bind Arrow C++ through the C Data Interface rather than writing a reader. This is the decision that keeps this milestone at roughly half the size it would otherwise be. A native Mojo Parquet reader is a performance project for M8, and a bound one is a capability that unblocks everything now.
