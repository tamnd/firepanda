...
- [x] `index`, `columns`, `dtypes`, `shape`, `size`, `ndim`, `empty`, `axes` (M6), where `empty` asks about either axis rather than about rows and `dtypes` is a column labelled by column name
- [ ] `values`, `flags`, `attrs` (M1), where `values` is the door out to numpy rather than a fact about the shape
...
- [x] `info` (M6), which reports the same facts in a shape a person reads rather than a shape a program reads, laid out exactly the way pandas lays it out, with the class and the spelling of the types and the memory number as the only lines that differ
...
- [x] `info` (M6), which is the frame's report with the position and the name taken out of the table and a `Series name:` line put above it, and where `verbose` and `max_cols` are accepted and change nothing because there is one column to list
...
- [x] `cat` with `sep` (M6), documented in `71-an-answer-that-is-narrower-than-a-column.md`, the first name on this accessor whose answer is narrower than a column rather than wider, which folds the whole thing into one string, dropping a missing row so that it takes its separator with it and keeping an empty row so that it does not, reading pandas' choice between those two off whether `na_rep` was given rather than off what it holds because `na_rep=""` is a different answer from leaving it out, and counting the bytes in one pass before writing them in another because a join is the one text operation whose answer length is known before it starts
- [ ] `cat` with `others` (M6), which aligns the two columns on their labels before it concatenates anything, so it is refused by name until alignment is written rather than served by position, which would answer a different question quietly
...
- [x] `get_dummies` (M6), documented in `72-an-answer-whose-width-is-in-the-data.md`, the third answer shape on this accessor and the first whose width comes out of the data rather than out of the arguments, splitting every row at a separator and answering one int64 column per distinct piece in byte order, keeping the empty string as a real column label because an empty row and a separator at either end both produce it, answering membership rather than a count so that a token seen twice is still a one, reading a missing row as a row of zeros which is the one place on this accessor a missing row does not stay missing, writing `dtype=bool` and refusing the rest by name, and answering a frame of no columns for a column of only missing rows where pandas' own frame constructor raises `ValueError: Empty data passed with indices specified.`
- [x] `normalize` (M6), documented in `74-the-one-name-answered-by-the-standard-library.md`, the one name on this accessor whose answers come from CPython rather than from Arrow because pandas defines it once as `unicodedata.normalize` a row at a time and Arrow has no normalization kernel to override it with, all four forms out of tables that are fully expanded so nothing recurses and a composition table derived by asking CPython which pairs it puts back together rather than by reimplementing the exclusion rules, Hangul answered by arithmetic with no table and an ASCII element answered as itself, and both of pandas' refusals reproduced in kind so that a form it does not know is a `ValueError` and a form that is not a string is a `TypeError`
- [ ] `encode`, `decode` (M6)
