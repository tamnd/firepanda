...
- [ ] All of `sum`, `mean`, `median`, `min`, `max`, `std`, `var`, `count`, `quantile`, `skew`, `kurt`, `sem`, `corr`, `cov`, `rank` on each of the three (M6), of which `sum`, `mean`, `count`, `min`, `max`, `var`, `std`, `sem`, `skew`, `kurt` and `median` are written on `rolling` and `expanding`
