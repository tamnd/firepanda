...
The short version is that the problem 07 called the hardest one is largely solved and cost 2.8 megabytes, the Arrow crossing that 07 treated as the centrepiece works today with zero copy proven by pointer identity, and the two items 07 spent one paragraph each on, error mapping and Ctrl-C, are the two that do not work at all. M3 should be reordered around that.
...
A built Mojo Python extension links three things. `@rpath/libKGENCompilerRTShared.dylib`, `@rpath/libAsyncRTMojoBindings.dylib`, and `/usr/lib/libSystem.B.dylib`. Those two Mojo libraries in turn pull in `libMSupportGlobals.dylib` and `libAsyncRTRuntimeGlobals.dylib`. That is the whole of it. Copied beside the extension, the four runtime libraries come to 2.72 megabytes and the extension itself is 170 kilobytes, so the complete self contained set is 2.8 megabytes.
...
**manylinux.** Everything above is macOS arm64. On Linux the runtime libraries are `.so` rather than `.dylib`, the rpath is patched with `patchelf` rather than `install_name_tool`, the token is `$ORIGIN` rather than `@loader_path`, and `auditwheel` will want to either bundle or be told to exclude the vendored Mojo libraries. None of that is hard and none of it is done, and the glibc version the Mojo runtime requires is unmeasured, which is the one item in it that could turn into a real constraint on which manylinux tag firepanda can claim.
