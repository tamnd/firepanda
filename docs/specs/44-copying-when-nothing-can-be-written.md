...
None of that applies here. There is no `__setitem__` on a frame or a series, there is no `assign`, and `inplace` is a rebind of the single slot a wrapper holds rather than a write into a buffer, which document 51 is the argument for. So a frame in this library cannot be written into at all, no object can ever observe a write to another object, and there is no later assignment for a deep copy to protect the original from. This paragraph originally rested on `inplace` being refused outright, and the conclusion is unchanged by its no longer being refused, because a rebind leaves every buffer exactly where it was.
