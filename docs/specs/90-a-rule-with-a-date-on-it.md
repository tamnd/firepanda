...
Nothing checks that `PYTHON_NEWEST` is the newest, as this is written. A CI job that fails when the interpreter it is running under is newer than the constant would turn the next release into a red light rather than into a quiet wrong answer, and that is worth doing the next time a version rule lands rather than now, when there is one of them. The next one landed and it is document 91, so the condition this paragraph set has been met and the job is document 92.
