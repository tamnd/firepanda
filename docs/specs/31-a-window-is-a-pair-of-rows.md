...
## 14. The eight that are here and the eighteen that are not

`sum`, `mean`, `count`, `min`, `max`, `var`, `std` and `sem`. They are eight rather than some other number because they are the ones a window can be carried through: the answer to a window can be derived from the answer to the window before it and the rows that changed. The first five carry a number and the last three carry a state, which section 6 is about.

`skew` and `kurt` carry the third and fourth moments, which are the same shape as the second and a different set of measurements, so they are their own piece of work rather than three more lines in `spread.mojo`. `median`, `quantile` and `rank` need the window sorted rather than folded, which is a different data structure again. `apply` needs a Python callable per window. `corr` and `cov` take a second column. `aggregate` is surface rather than kernel, and so is a `numeric_only` that drops columns rather than refusing them. The exponentially weighted window has no edges at all and shares nothing with any of this. A window given as a duration rather than a count needs a datetime index to measure against, and a window under a group by needs the row numbers restarted per group.
