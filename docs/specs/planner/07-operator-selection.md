...
An inner join in `firepanda/join/pairs.mojo` already builds from the smaller side, because the hash table is what has to be resident: `BUILD_SIDE_MARGIN` is how much smaller the left has to be before the sides swap, `_by_left_row` is the counting sort that puts the output pair order back, and the constant carries its measurement. What still goes by the parameter name is a left, semi, anti and outer join. An inner join is the one kind where exchanging the sides changes nothing about which rows come out; the others each have to answer about one named side, and a swap would change which side that is, so a swap for them means keeping track of the exchange all the way to the output rather than only counting rows at the door. A right join is already the left join with the sides exchanged, so it is the same problem seen from the other end. DuckDB has this as a distinct optimizer pass from join ordering.
...
That last route reads like the gap and it was written up here as one, and the measurement says it is not. It is a whole column pass over left plus right where the dictionary route is a pass over the smaller side and a lookup per row of the larger, and it copies every key column of both sides to build the concatenation it factorizes, which all sounds expensive. What that description leaves out is that the copy is a parallel memcpy and that `group_ordinals` on the other end has tuple handling of its own: an integer pair no table can be laid over still gets one fused hash of the whole tuple in a single parallel pass, and a tuple with a string in it gets a factorize a key and a parallel fold.

The streaming join in `firepanda/exec/node.mojo` does get out of that route, but out of necessity rather than because it is slow. It cannot concatenate both sides because it only ever has one of them, so it packs the key tuple into one byte string per row instead, which needs no range and so needs no second frame, and then it is a join on one text key and takes the dictionary route. `firepanda/join/packed.mojo` is the packing.

Doing the same in `align_keys` was written down here as owed, and it was built and measured and is not owed. At ten million probe rows the byte packing is five to eight times slower than the concatenating route on every shape where the two sides differ in height, on an integer pair too far apart to pack and on a pair with a string in it alike, and the two are interleaved where the sides are the same height. The table is in the module docstring of `firepanda/join/keys.mojo` and the six benchmark rows it was taken on are in the suite. The reason is the paragraph above: the route it had to beat spreads its work and the byte packing writes its byte strings in a serial pass. If that pass is ever spread across cores the question is worth asking again, and only on the equal sides rows.

So the real gap in this area is not the compound key. It is that every route here builds its table on one thread while the route it competes with factorizes on all of them, which is the same sentence `STRING_BUILD_SHARE` is there to work around and the reason the equal sides rows are slow on every route at once.
