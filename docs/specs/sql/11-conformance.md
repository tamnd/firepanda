...
Three of the twenty two queries are in the list today. q1 and q3 agree with DuckDB row for row, and q6 is refused for the decimal literals in `l_discount BETWEEN 0.05 AND 0.07`. The other nineteen want a dependent join, a decimal, or both, and each one goes in the list the day it runs rather than sitting there as a pending failure.
