...
The two ways a probe can agree are counted separately, because they are not the same claim. 14,530 of the expressions have their type compared and that is the claim the harness is for. 112 of them, across nine names, are calls returning a container, where a `T[]` or a `MAP` needs an element type `SqlType` does not carry, so what is compared is which overload won and not what it returns. Folding the two into one figure would give a number that improves when the binder is taught less, so the report prints both and lists the nine names. Issue #780 is what is left of that list; it started at 932 across 29 names, and the template letters, the bare decimals and the `sum`, `avg` and `median` rules have since been derived.
