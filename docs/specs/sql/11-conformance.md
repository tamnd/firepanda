...
The two ways a probe can agree are counted separately, because they are not the same claim. 13,710 of the expressions have their type compared and that is the claim the harness is for. 932 of them, across 29 names, are calls whose declared return type is a rule over the arguments rather than a type, so what is compared is which overload won and not what it returns. Folding the two into one figure would give a number that improves when the binder is taught less, so the report prints both and lists the 29 names, and that list is the work that is left: mostly the template letter that means the argument's own type, which is `abs`, `floor`, `round`, `min`, `max` and the rest of that shape, plus the derived rules under `sum` and `avg` and the containers under `list` and `histogram`. Issue #780 carries the three groups and the order to do them in.
