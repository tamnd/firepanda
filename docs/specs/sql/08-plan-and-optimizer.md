...
TPC-H q17 answers on that, where it used to refuse. q2 is past the names and stops on a cross join whose right side is more than one row, and q20 is past them and stops on a left join with two key pairs. Both are operator limits and neither has anything to do with names.
