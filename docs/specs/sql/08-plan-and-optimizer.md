...
Recognising the correlation is not the same as lowering it, and the three TPC-H queries above stop past it rather than on it. `_folded_join` lowers the subquery's `FROM` into the scope the outer query is using, which is what puts both sides of the correlation in reach at once, and a table named on both sides then collides in that one scope. SQL's rule is that the inner one shadows the outer, which wants a scope that knows which level each name was added at and a bare column under one pinned to the innermost relation that has it rather than left for binding to find on both sides. That is q2 and q17. q20 is past both and stops on a left join with two key pairs, which is an operator limit and has nothing to do with names.
