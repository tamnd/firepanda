...
Until the plan carries an exact decimal, `firepanda/sql/plan.mojo` refuses a decimal literal whose own type would be the answer's, which is `SELECT 1.1` and `SELECT 1.1 + 2.2`. A refusal is visible in `pixi run sql-support` and in the conformance harness, and a double that answers `3.3000000000000003` is not visible anywhere. Written against a column the literal does lower, because the column is not a decimal either and DuckDB casts the literal to a double at that same point. `_lower_operand` folds the literals around it first, in the scaled integers a decimal really is, and converts the fold once, so `l_discount BETWEEN 0.06 - 0.01 AND 0.06 + 0.01` compares against the doubles nearest five and seven hundredths rather than against 0.049999999999999996 and a number below seven hundredths. One case is left where the conversion is visibly not what DuckDB has, and it is recorded in `tests/differential/answers.mojo` rather than hidden. A cast to an integer is a boundary where the far side is not a double either, so DuckDB never converts and rounds the decimal straight to the integer, away from zero, where firepanda rounds the double it already holds to even. `CAST(5 * 2.50 AS BIGINT)` is 13 there and 12 here, and 12 is what DuckDB itself answers for `CAST(12.5e0 AS BIGINT)`. The two rules only part on a tie. Adding `DECIMAL` and `INT128` to `LogicalType` is the work that removes both that case and what is left of the refusal, and it is engine work rather than SQL work for the same reason as section 1.1.
...
Before any of that, something has to decide a subquery is correlated at all, and it has to decide before the subquery's own `FROM` is lowered, because the answer picks which of two shapes to build. There are two ways the reach is written and they are not asked the same question. A name written in front of a column is a relation name, and a qualifier that is not one the subquery's own `FROM` put in reach is correlation, which is answered off the text because the relation names on both sides are written down. A bare column name says nothing on its own. SQL resolves it inside first and only then outside, so a name the subquery's own `FROM` produces is the inner one and is not correlation, and a name it does not produce is the outer one and is. Answering that means reading what the inner `FROM` produces out of the catalog, which works for a named table and for a join of them, and does not work for a CTE, a subquery, a table function or a set operation, whose column lists are decided by lowering them. Where the columns cannot be read the question is left unasked and the subquery reads the way it read before, which is a refusal rather than a wrong answer. TPC-H q2, q17 and q20 all write their correlation as a bare name, so the bare half is not an edge case.

Lowering it then puts two queries' names in one scope, because `_folded_join` lowers the subquery's `FROM` into the scope the outer query is using, which is the only place a condition reading both sides of the correlation can be written. A table named on both sides is two relations and not one, and SQL's rule for them is that the inner shadows the outer rather than colliding with it. The scope draws a line, `floor`, marking where the innermost query's names begin. A repeat is refused above the line, which is two relations of the same name in one `FROM` and a query that cannot say which it means, and allowed across it. A relation name is resolved from the end, so the innermost query answers first. A bare column written under the line goes to the innermost relation in reach that has it, rather than being left for binding to find, because binding sees one list of columns and cannot tell a name two queries have from a name one query has twice. Two relations within the same query having it is a real ambiguity and stays binding's to report, and so does a derived table, which has no relation number to pin to for the reason the qualified path gives. The line is saved and put back beside how far reach went, so a correlated subquery inside a correlated subquery nests rather than flattens.

TPC-H q17 and q2 answer on that, where both used to refuse. q2 needed one more thing after the names, which was for join ordering to look a node further down: decorrelating its subquery joins the answer on above the `FROM` that subquery correlates with, so the comma chain that has the bad order in it is the left input of that join rather than the node the filter reads, and the ordering pass was only ever looking at what the filter reads. q20 answers too. It was past the names already and stopped on a left join with two key pairs, which was an operator limit and had nothing to do with names: the streaming join built its table from one column, and a left join cannot be made to pair on the first key and test the rest afterwards because a filter above the join cannot tell a row that matched nothing from a row it is dropping. The operator packs the tuple into one byte string per row on each side now, which needs only the side being packed, and then it is the text key it already had.
...
Constant folding, with document 06's exact semantics, so folding `1.1 + 2.2` must produce `DECIMAL(3,1) 3.3` and not a double, or the optimizer changes the answer. Comparison simplification. Constant condition `CASE` elimination. `IN` over a short constant list becomes a chain of `OR`, and over a long one a hash set. `LIKE` with no wildcards becomes equality, with a trailing `%` becomes `starts_with`, and with leading and trailing becomes `contains`, all of which lower directly onto the kernels already in `firepanda/kernel/pattern.mojo`, where they are documented as LIKE lowerings. A pattern that is none of those shapes lowers onto the general matcher in the same file, which is a walk rather than a search and is the reason the shapes above it are tried first. Regex becomes `LIKE` where the pattern is anchored and literal. Common subexpression elimination within a tree means `x*2` appearing three times is computed once.
