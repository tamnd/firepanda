...
It pays for itself four times. Round trip is the transformer's main test: parse, print, reparse, compare, over 4,046 files, and it catches precedence bugs that nothing else catches. It is the fuzzer's oracle: generate a random AST, print it, parse it, compare. It is how `EXPLAIN` shows filter and projection expressions, which is how a user debugs a pushdown that did not happen. And it is what names an output column the query did not name, so `SELECT sum(x) FROM t` comes back with a column called `sum(x)`.

That last one is why the printer is reached from the lowering rather than only from the front doors. DuckDB names a column this way too, and with the same printer it parses with, so the two agree on most of it without either side aiming at the other. Both normalize, so `SUM( x  )` is `sum(x)` in both. Both parenthesize every operand, so `x * 2 + 1` is `((x * 2) + 1)` in both. Neither makes a duplicate unique. What still differs is `count(*)`, which DuckDB names `count_star()`, negation, which DuckDB names `-(x)`, a cast, which DuckDB names by the type it resolved to rather than by the type text that was written, a `CASE`, where DuckDB writes in the `ELSE` it filled in, and a call to a function whose name is a keyword, which DuckDB qualifies with the schema it found it in. All five are the printer disagreeing rather than the rule disagreeing, and closing them means a second printer that prints for a name rather than for a reparse.
