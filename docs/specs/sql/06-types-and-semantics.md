...
The cast of a float to an integer is the same problem one layer down and it was settled the other way. `CAST(2.6 AS BIGINT)` is 3 in DuckDB, which rounds to the nearest whole number with a tie going to the even one, so `7.5` and `8.5` are both 8 and `-7.5` is `-8`. `astype` truncates towards zero, which is pandas. That was issue #786 and the fix is a flag the caller passes rather than a second kernel, because the conversion loop is already instantiated once for every pair of types and doubling that to change one line of it would cost more compile time than the whole rest of the module. The flag is set on the plan's cast by the SQL lowering whenever the target is an integer, and the loop tests at compile time that the pair has a fraction to lose, so every pair that does not compiles to what it compiled to before.
